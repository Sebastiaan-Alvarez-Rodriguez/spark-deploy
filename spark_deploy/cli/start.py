...
def subparser(subparsers):
    ...
    startparser.add_argument('--workdir', metavar='path', type=str, default=defaults.workdir(), help='Path to Spark workdir location for all slave daemons (default={}). Note: The home directory of the remote machines is prepended to this path if it is relative.'.format(defaults.workdir()))
...
def deploy(parsers, args):
    ...
    return _start(reservation, args.install_dir, args.key_path, args.master_id, master_host=args.master_host, master_port=args.master_port, webui_port=args.webui_port, slave_workdir=args.workdir, silent=args.silent, retries=args.retries) if reservation else False